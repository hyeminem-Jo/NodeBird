    ...
    return (
      <>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <img role="presentation" style={{ width: "49.2%", display: 'inline-block' }} src={`http://localhost:3065/${images[0].src}`} alt={images[0].src} onClick={onZoom} />
          <img role="presentation" style={{ width: "49.2%", display: 'inline-block' }} src={`http://localhost:3065/${images[1].src}`} alt={images[1].src} onClick={onZoom} />
        </div>
        {/* role="presentation" : 시각장애인을 위한 배려() */}
        {/* 스크린리더에서 굳이 클릭할 필요 없는 것일 때 */}
